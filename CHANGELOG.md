...
- **klipperscreen** module: chroot/CI mounts can be **nosuid** — running `KlipperScreen-install.sh` as `pi` + `sudo apt` fails; patch installer (allow root, strip `sudo`), run as root with `HOME`/`USER` set to `pi`, **`NETWORK=n`** (previous value was not recognized and could trigger NM + **reboot**); `git clone` as root then `chown`

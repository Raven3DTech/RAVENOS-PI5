...
### Changed
- **Configurator install path & Moonraker:** the Next app lives at **`~/configurator`** (not `~/ratos-configurator`). **systemd** unit is **`ravenos-configurator.service`**; Moonraker uses **`[update_manager ravenos_configurator]`** with **`managed_services: ravenos-configurator`**. Fork **[Raven3DTech/RatOS-configurator](https://github.com/Raven3DTech/RatOS-configurator)** ships RavenOS UI theming (violet palette, titles) and script renames aligned with this layout.
- **Configurator fork:** image build clones and Moonraker’s configurator update-manager **`origin`** is **[Raven3DTech/RatOS-configurator](https://github.com/Raven3DTech/RatOS-configurator)** (fork of [Rat-OS/RatOS-configurator](https://github.com/Rat-OS/RatOS-configurator), branch `v2.1.x`). Periodically merge or rebase **`upstream/v2.1.x`** (or sync from Rat-OS) to pick up Rat-OS fixes.
...
- **RatOS-style web entry:** nginx on port **80** reverse-proxies **`/configure/*`** to RavenOS Configurator (Next on **:3000**); **`/config`** and **`/config/`** redirect to **`/`** so RatOS install-doc links work. Mainsail sidebar panel, **`.env.local` `NEXT_PUBLIC_MOONRAKER_URL`**, and docs use **`http://<host>/configure`** with same-origin Moonraker via nginx. **systemd** runs Next from **`~/configurator/src`** with the correct **`node_modules`** path. First boot updates **`.env.local`** in **`src/`** (and root if present).
